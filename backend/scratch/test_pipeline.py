...
def test_e2e_pipeline():
    ...
    print("==================================================")
    print("STEP 1: Uploading sample DOCX...")
    print("==================================================")
    ...
    print("\n==================================================")
    print("STEP 2: Starting conversion (Validation + Pandoc)...")
    print("==================================================")
    response = client.post("/convert", json={"job_id": job_id})
    assert response.status_code == 200, f"Conversion trigger failed: {response.text}"
    job_data = response.json()
    print(f"Conversion step response:")
    print(f"Status: {job_data['status']}")
    print(f"Progress: {job_data['progress']}%")
    print(f"Current Step: {job_data['current_step']}")
    
    # Check if conversion succeeded or failed (e.g. if Pandoc isn't installed)
    if job_data["status"] == "FAILED":
        print("\nPipeline failed at conversion phase. Errors:")
        print(job_data["errors"])
        print_temp_logs(job_id)
        sys.exit(1)

    print("\n==================================================")
    print("STEP 3: Compiling LaTeX to PDF...")
    print("==================================================")
    response = client.post("/compile", json={"job_id": job_id})
    assert response.status_code == 200, f"Compilation trigger failed: {response.text}"
    job_data = response.json()
    print(f"Compile step response:")
    print(f"Status: {job_data['status']}")
    print(f"Progress: {job_data['progress']}%")
    print(f"Current Step: {job_data['current_step']}")
    
    if job_data["status"] == "FAILED":
        print("\nPipeline failed at compile phase. Errors:")
        print(job_data["errors"])
        print_temp_logs(job_id)
        sys.exit(1)
    ...
    # Verify metadata and file existence
    ...
    for log_name in ["system.log", "pandoc.log", "compiler.log"]:
        ...
        if log_file.exists():
            ...
            print(f"\nWarning: Log file {log_name} does not exist.")

def print_temp_logs(job_id: str):
