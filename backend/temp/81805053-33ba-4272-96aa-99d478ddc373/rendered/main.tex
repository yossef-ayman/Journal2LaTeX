...
\author{G.M. Vijayalakshmi$^{1}$, G. Susila*$^{1}$, Z. Che Muda$^{2}$, Sudesh Nair Baskara$^{2}$, P. Roselyn Besi$^{3}$, Ali Akgül$^{4}$, Numerical Simulation, Behavioural epidemiology, Mental health intervention., Contents, 1. Introduction, 2. Fractional Elementary Concepts, 3. Model Formulation, 4. Stability analysis, 5. Optimal Control Theory, 6. Numerical simulation, 7. Conclusion, The basic definitions are presented related to fractional calculus., Definition 2.1 RLFI [Riemann – Liouville fractional integral], Definition 2.2 CFOD [Caputo – Fractional Order Derivative], The function's Caputo fractional-order derivative is}
