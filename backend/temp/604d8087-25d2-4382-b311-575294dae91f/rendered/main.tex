...
\makeatletter\@ifundefined{providecolor}{}{%
  \providecolor{LimeGreen}{RGB}{50,205,50}
  \providecolor{PineGreen}{RGB}{21,133,110}
  \providecolor{abstractcolor}{RGB}{128,128,128}
  \providecolor{rulecolor}{RGB}{128,128,128}
}\makeatother
% <<< j2l compat
\documentclass[10pt]{NSP1}
% The dvips named colours the class uses (LimeGreen, PineGreen, ...) are
% supplied generically by the compilation compatibility layer, so they are not
% hardcoded here.  Only the journal's banner tint is set, to match the printed
% original's orange masthead (the class default would render it green).
...
\IfFileExists{keyval.sty}{\usepackage{keyval}}{}
\makeatletter
...
\author{G.M. Vijayalakshmi$^{1}$, G. Susila*$^{1}$, 3P.Roselyn Besi, *Ali Akgül$^{2}$, Numerical Simulation, Behavioural epidemiology, Digital health intervention., Contents, 1. Introduction, 2. Fractional Elementary Concepts, 3. Model Formulation, 4. Stability analysis, 5. Optimal Control Theory, 6. Numerical simulation, 7. Conclusion, The basic definitions are presented related to fractional calculus., Definition 2.1 RLFI [Riemann – Liouville fractional integral], Definition 2.2 CFOD [Caputo – Fractional Order Derivative], The function's Caputo fractional-order derivative is}
...
\begin{figure}[htbp]
\centering
\ifdim 1.3576in>\linewidth
  \includegraphics[width=\linewidth,keepaspectratio]{media/image7.png}
\else
  \includegraphics[width=1.3576in,height=2.3285in]{media/image7.png}
\fi
\end{figure}
...
\begin{figure}[htbp]
\centering
\ifdim 0.8403in>\linewidth
  \includegraphics[width=\linewidth,keepaspectratio]{media/image9.png}
\else
  \includegraphics[width=0.8403in,height=2.6395in]{media/image9.png}
\fi
\end{figure}
...
\else
  \includegraphics[width=5.2069in,height=3.6465in]{media/image8.png}
\fi
