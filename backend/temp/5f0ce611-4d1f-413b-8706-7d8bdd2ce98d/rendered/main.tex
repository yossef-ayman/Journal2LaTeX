...
% <<< j2l
	...
	\title{Fractional-Order Dynamics of Cardio-Respiratory Oxygen Transport: A Stable Regulator of Human Endurance and the Boundary of Chaos}
	
	\author{Aashima Bangia$^{1}$ \and Rashmi Bhardwaj$^{2}$ \and Asokan Vasudevan$^{3}$ \and Naeema Darwish Khamis Al Maashari$^{4}$ \and Yafang Yan5 Khin Yadanar Htun$^{5}$ \and Email:  ORCID:$^{5}$ \and Email:$^{5}$ \and Email:  ORCID:$^{6}$ \and AMS Classification: 37M$^{5}$ \and 37M$^{5}$ \and 37Fxx$^{5}$ \and 65-XX$^{5}$ \and Physiological basis of the model assumptions$^{5}$ \and Selection of parameter values$^{5}$ \and leading to$^{5}$ \and f can be approximated through Jacobian matrix as:$^{5}$ \and For the system under study$^{5}$ \and the jacobian$^{5}$ \and J can be intended as:$^{5}$ \and Consecutively$^{5}$ \and Characteristic Polynomial$^{5}$ \and P(n) so formed is:$^{5}$ \and Definition: Grunwald-Letnikov Derivative$^{5}$ \and Binomial coefficient present in the derivative can be computed through Gamma function as:$^{5}$ \and Theorem: Riemann Liouville Fractional Derivative (RLFD)$^{5}$ \and Riemann alongwith Liouville in the year 1847 developed this derivative as:$^{5}$ \and having  as the well-known gamma function computed through:$^{5}$ \and Now$^{5}$ \and employing fractionalization algo for extending this fractional equation i.e.$^{5}$ \and as$^{5}$ \and the$^{5}$ \and Whenever$^{5}$ \and the RL operator reduces the equation to:$^{5}$ \and This is equivalent to the reduced RLFD given in equation (3).$^{5}$ \and Definition: Caputo Derivative for Fractional order$^{5}$ \and Caputo derivative is defined for fractional order$^{5}$ \and as:$^{5}$ \and Definition: Riemann-Liouville Fractional Integral(RLFI)$^{5}$ \and Definition: Caputo Fractional Integral$^{5}$ \and The integral for the order$^{5}$ \and defined in the sense of Caputo as:$^{5}$ \and having  as the well-known gamma function computed through:$^{5}$ \and having initial conditions:$^{5}$ \and where denotes the operator as defined by the Caputo as:$^{5}$ \and Consider the integer order general form of differential equations represented as:$^{5}$ \and having initial conditions:$^{5}$ \and which leads to the integration results derived given in [] as follows:$^{5}$ \and Rearranging the integration eqn. (16) as follows:$^{5}$ \and On employing trapezoidal rule on eqn. (16) alongwith (17)$^{5}$ \and it becomes$^{5}$ \and then eqn. (16) becomes:  such as .$^{5}$ \and Thus$^{5}$ \and Therefore$^{5}$ \and eqn. (20) in terms of eqn. (22) becomes:$^{5}$ \and And the predictor form$^{5}$ \and of this fractional Adams-Moulton method can be seen as:$^{5}$ \and This implies that as per the Adams-Moulton method is represented as:$^{5}$ \and Phase$^{5}$ \and Variation in q = [q$^{5}$ \and q$^{5}$ \and q3]$^{5}$ \and Variation in time period$^{5}$ \and t$^{5}$ \and Stable$^{5}$ \and Critical (extended model)$^{5}$ \and Chaotic (extended model)$^{5}$ \and Fractional order$^{5}$ \and q$^{5}$ \and |arg λ| min (rad)$^{5}$ \and qπ/$^{5}$ \and Regime$^{5}$ \and Yes$^{5}$ \and Stable focus$^{5}$ \and Yes$^{5}$ \and Stable focus$^{5}$ \and Yes$^{5}$ \and Stable focus$^{5}$ \and Yes$^{5}$ \and Stable focus$^{5}$ \and Yes$^{5}$ \and Stable focus$^{5}$ \and Yes$^{5}$ \and Stable focus$^{5}$ \and Yes$^{5}$ \and Stable focus$^{5}$ \and Table-2: Values of parameters considered in the model$^{5}$ \and Parameter$^{5}$ \and Description$^{5}$ \and Values$^{5}$ \and Mr$^{5}$ \and Rate of Metabolism$^{5}$ \and 0.01/sec$^{5}$ \and V$^{5}$ \and Maximum lung volume$^{5}$ \and 3600ml$^{5}$ \and Maximum transmission rate of molecules from lungs to bloodstream$^{5}$ \and 0.1/sec$^{5}$ \and Maximum transmission rate of molecules from bloodstream to bodycells$^{5}$ \and 0.7/second ()$^{5}$ \and Breathing-period$^{5}$ \and 20s inhale$^{5}$ \and 20s suspension$^{5}$ \and 20s exhale i.e.$^{5}$ \and 60 secs in total$^{5}$ \and Amount of oxygen intake under different circumstances$^{5}$ \and 24L/minute ()$^{5}$ \and Table 3: Literature studies for the kinesiology-based models$^{5}$ \and Year$^{5}$ \and Author$^{5}$ \and Description$^{5}$ \and Parameters$^{5}$ \and Results$^{5}$ \and Complexity Dynamics of Meditating Body$^{5}$ \and Differential model developed for the human body$^{5}$ \and Analysis of the human body during different meditation phases to capture varied oxygen concentration-levels$^{5}$ \and Teka W.W. et al$^{5}$ \and Spiking as well as overflowing patterns of fractional-order Izhikevich model$^{5}$ \and Fractional order equations developed for Izhikevich model$^{5}$ \and Analyzed the parameters of the fractional order apt for simulation$^{5}$ \and Dynamical Indicator of Human Body’s Physical Endurance$^{5}$ \and Fractional ordered SEQIR-based model was developed to study Coronavirus dynamics$^{5}$ \and Numerical simulations plotted$^{5}$ \and Lyapunov stability found the system to be asymptotically stable$^{5}$ \and Ghita M.$^{5}$ \and Copot D.$^{5}$ \and Ionescu C.M.$^{5}$ \and Lung cancer dynamics using fractional order impedance modeling on a mimicked lung tumor setup$^{5}$ \and Fractional order impedance modelling was developed to study lung tumor$^{5}$ \and Li Z$^{5}$ \and Pei Y$^{5}$ \and Wang Y$^{5}$ \and Tian Q$^{5}$ \and An enhanced respiratory mechanics model based on double-exponential$^{5}$ \and fractional calculus$^{5}$ \and Fractional respiratory mechanics fitted to real data$^{5}$ \and The hybrid model achieved significantly better results for different ventilation modes$^{5}$ \and Alharbi. N$^{5}$ \and Present study$^{5}$ \and Fractional Ordered Kinesiology Model$^{5}$}
	\institute{$^{1}$ Assistant Professor, Sharda University, Greater Noida, U.P.-201310. \\ $^{2}$ Professor of Mathematics, Head, Non-Linear Dynamics Research Lab, \\ $^{3}$ Faculty of Business and Communications, INTI International University, Negeri Sembilan 71800, Malaysia. \\ $^{4}$ University School of Basic and Applied Science, GGSIPU, Delhi \\ $^{5}$ This derivative is used to describe the models with complexities as RLFD becomes incompetent to deal with the intricate systems. \\ $^{6}$ Faculty of Liberal Arts, Shinawatra University, Thailand, email: email:}
	\received{}
	\revised{}
	\accepted{}
	\published{}
	
	\abstracttext{This work develops a fractional-order dynamical model of coupled cardio-respiratory oxygen transport among the lungs, the heart and bloodstream, and the body tissues, and studies the conditions under which the human body sustains or loses stable oxygen regulation during sustained effort. The three-compartment system is formulated with Caputo fractional derivatives of order q, so that physiological memory arising from delayed oxygen uptake, circulatory transport lags, and gradual tissue utilisation is represented intrinsically rather than through added delay terms. The equilibrium structure is derived, and the associated Jacobian matrix is obtained in closed form as a real, non-symmetric operator. Local stability is established rigorously using the Matignon criterion appropriate to fractional-order systems: at the physiological operating point the eigenvalues give a minimum argument of 1.605 radians, yielding a critical order of 1.02, so the regulated state is a stable focus for every admissible order between zero and one. This analytical result is confirmed numerically by the largest Lyapunov exponent, computed with a variational method validated on the classical Lorenz system, which returns a value of -0.10 independent of the initial condition. The model therefore describes a robustly self-regulating oxygen-transport system, and this stable regime is interpreted as the dynamical basis of endurance. A physiologically motivated extension in which the cardiac response gain is separated from and exceeds the pulmonary transfer coefficient and the metabolic dissipation is raised to realistic levels undergoes a genuine transition to chaotic dynamics, offering a candidate description of cardio-respiratory dysregulation and fatigue. Notably, stronger memory (a lower fractional order) suppresses this chaos, indicating that the fractional formulation enhances the robustness of the regulated endurance state. A parameter-sensitivity analysis confirms that stability persists across the physiological ranges of the transport, breathing, and metabolic parameters. The findings connect fractional-order nonlinear dynamics to measurable endurance and recovery in sports science, physiotherapy, and cardio-respiratory health monitoring. Numerical simulations employ a fractional Adams–Bashforth–Moulton (ABM) scheme.}
	
	\keywords{Caputo fractional derivative, Cardio-respiratory coupling, Chaos, Largest Lyapunov exponent, Oxygen transport, Fractional order Differential Equations (FODE), Fractional order Modelling, Adam Bashforth Moulton (ABM)}
	
	\mail{18778460649@163.com}
	\maketitle
...
\bibitem{ref13}
Hasan M.N, Uddin M.S., Biswas M.H.A. (2020). Interactive Eﬀects of Disease Transmission on Predator-Prey Model. Journal of Applied Nonlinear Dynamics. 9(3), 401-413.
